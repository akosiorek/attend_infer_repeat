...
if init_explore_eps is not None:
...
if prior_weight > 0.:
    ...
    if num_steps_prior is not None:  
        ...
        prior = np.asarray([.25] * 4, dtype=np.float32)
...
def make_fig(checkpoint_dir=None, global_step=None, max_imgs=10):
    xx, pred_canvas, pred_crop, prob, pres, w = sess.run([x, prob_canvas, cropped, presence_prob, presence, where])
    ...
    for i, ax_row in enumerate(axes[1+n_steps:]):
        for j, ax in enumerate(ax_row):
            ...
            ax.set_title('{:d} with prob {:.02f}'.format(int(pres[i, j, 0]), prob[i, j, 0]), fontsize=4*scale)
